# 🎮 Kahoot-Style AI Trivia Game — Project Plan

A real-time multiplayer trivia game inspired by Kahoot. **Play with Friends** pits players against each other across 15 timed questions — scoring rewards both accuracy and speed. **Solo Mode** lets you play endlessly and climb a persistent global leaderboard. Questions are generated by the Claude API. Built with React, Node.js, Socket.io, and PostgreSQL.
...
### 1. GitHub
- You already have an account — create a new repository called `trivia-game`
...
- **Strategy:** Pre-generate questions in bulk with a one-time script and store them in the DB — zero API calls during gameplay
...
- Railway gives you a public URL — copy this into your frontend env vars
...
│   ├── src/
│   │   ├── components/
│   │   │   ├── Login.jsx            # Simple login/register screen (first thing users see)
│   │   │   ├── Home.jsx             # Main menu: "Play with Friends", "Solo", "Leaderboard"
│   │   │   ├── Lobby.jsx            # Multiplayer: create/join room, waiting room, player list
│   │   │   ├── MultiGame.jsx        # Multiplayer game: Kahoot-style timed questions
│   │   │   ├── MultiResults.jsx     # Multiplayer end screen: final leaderboard for that game
│   │   │   ├── Solo.jsx             # Solo mode: endless questions, quit anytime
│   │   │   ├── SoloResults.jsx      # Solo end screen: correct count, then global leaderboard
│   │   │   └── Leaderboard.jsx      # Global leaderboard page (standalone)
│   │   ├── App.jsx
│   │   └── main.jsx
│   └── package.json
│
...
│   ├── rooms.js               # In-memory multiplayer game state management
│   ├── scoring.js             # Kahoot-style score calculator
│   ├── db.js                  # PostgreSQL connection pool
│   ├── routes/
│   │   ├── auth.js            # POST /auth/register, POST /auth/login
│   │   ├── questions.js       # GET /questions/random (for solo mode)
│   │   └── leaderboard.js     # GET /leaderboard, POST /leaderboard (solo score submission)
│   └── package.json
│
├── scripts/
│   └── generate-questions.js  # One-time script: uses Claude to fill DB with trivia questions
│
└── README.md
```
...
With 15 questions, the theoretical max score is **15,000 points**. The spread between fastest and slowest correct answer on a single question is only 150 points — enough to matter across a full game, but a slow correct answer always beats a wrong one.
...
### Solo Mode — Endless Questions, No WebSockets

Solo is pure HTTP. No rooms, no real-time — just fetch, answer, repeat.

**Flow:**

1. Player clicks **Solo** on the home screen
2. Frontend makes `GET /questions/random?count=10` — server returns 10 random questions
3. Player answers one question at a time, no timer enforced (casual pace)
4. When 10 questions are used up, frontend silently fetches 10 more in the background — the player never sees a loading screen
...
**Why endless?** It differentiates solo from multiplayer. Solo is the chill mode — no pressure, no time limit, play during a bus ride. The global leaderboard gives it a reason to keep playing.

---

### Multiplayer Mode — Kahoot-Style, 15 Questions, Timed
...
```
 Create Room  ──or──  Join Room (enter code)
      │                      │
      ▼                      ▼
         Lobby (waiting room)
         - Shows player list
         ...
         └───────────────────┘
                  │
                  ▼ (everyone answered or timer expired)
         ┌───────────────────┐
         │  Answer Reveal     │
         │  Correct: B ✓      │
         │                   │
...
- **850–1000 points** for a correct answer depending on speed; 0 for wrong/timeout
...
- Multiplayer scores do **not** affect the global leaderboard — it's a separate competitive space
...
```
CLIENT → SERVER:
  create-room     { username }                    → host creates a new room
  join-room       { roomCode, username }          → player joins existing room
  start-game      { roomCode }                    → host starts the game
  submit-answer   { roomCode, answerIndex, timestamp }  → player submits their pick

SERVER → CLIENT:
  room-created    { roomCode }                    → sent to host with their room code
  room-joined     { players }                     → sent to all in room (updated player list)
  player-joined   { username }                    → broadcast when someone new joins
  game-starting   { totalQuestions: 15 }          → countdown before first question
  new-question    { questionIndex, question, options, startTime }  → next question for everyone
  time-up         { }                             → 20 seconds expired, stop accepting answers
  answer-reveal   { correctIndex, scores, standings }  → shows correct answer + updated rankings
  game-over       { finalStandings }              → final leaderboard for the session
  player-left     { username, players }           → someone disconnected
```
...
4. Timer expires (or all answered) → server emits `time-up`
...
7. If more questions remain → go to step 1
   If question 15 is done → emit `game-over` with final standings
```
...
// createRoom(hostUsername) → roomCode
// joinRoom(roomCode, username) → room
// startGame(roomCode, questions) → void
// submitAnswer(roomCode, username, answerIndex, timestamp) → { score, allAnswered }
// advanceQuestion(roomCode) → { question, isGameOver }
// getStandings(roomCode) → sorted array of { username, score }
...
## 🤖 AI Question Generation with Claude
...
async function main() {
  const TOTAL_BATCHES = 10; // 10 batches × 20 questions = 200 questions
  let totalInserted = 0;
...
**Server** — create `.env` in `/server` (never commit to GitHub):
...
- [ ] Build `db.js` — PostgreSQL connection pool using `DATABASE_URL`
- [ ] Build `POST /auth/register` — hash password with bcrypt, insert user, return JWT
- [ ] Build `POST /auth/login` — verify password, return JWT
- [ ] Build `Login.jsx` — simple form with username/password, toggle between login/register
...
- [ ] Build `Home.jsx` — three buttons: "Play with Friends", "Solo", "Leaderboard"
- [ ] Set up React Router with routes:
  - `/login` → Login.jsx
  - `/` → Home.jsx (redirects to /login if no JWT)
  - `/solo` → Solo.jsx
  - `/lobby` → Lobby.jsx
  - `/game` → MultiGame.jsx
  - `/results` → MultiResults.jsx
  - `/solo-results` → SoloResults.jsx
  - `/leaderboard` → Leaderboard.jsx
- [ ] Add a simple navbar/header with the player's username and a logout button

### Phase 5 — Solo Mode (Days 5–6)
...
- [ ] Build `POST /leaderboard` route:
  ...
  ```
- [ ] Build `GET /leaderboard` route — returns top 20 by `total_correct`
- [ ] Build `Leaderboard.jsx` — standalone page accessible from home screen
- [ ] **Test:** play solo, quit after some questions, verify score appears on leaderboard

### Phase 6 — Multiplayer: Lobby (Days 7–8)
...
- [ ] Build `Lobby.jsx`:
  - "Create Room" button → emits `create-room`, shows room code prominently
  - "Join Room" input + button → emits `join-room` with entered code
...
- [ ] Handle disconnections — remove player from room, emit `player-left`
- [ ] **Test:** open two browser tabs, create a room in one, join in the other, see both names appear

### Phase 7 — Multiplayer: Game (Days 9–11)
...
- [ ] Implement `submit-answer` event:
  ...
  - Check if all players have answered → if yes, trigger early reveal
...
- [ ] Build `MultiGame.jsx`:
  - Receives `new-question` → shows question + 4 colored answer buttons + countdown timer
  - On answer click → emit `submit-answer`, disable buttons, show "waiting for others..."
  - Receives `time-up` → disable buttons if not already answered
  - Receives `answer-reveal` → highlight correct answer, show points earned, show standings
  - After 5 seconds → ready for next question
  - Receives `game-over` → navigate to results
...
### Phase 8 — Polish & Deploy (Days 12–14)

- [ ] **UI Polish:**
  - Color-coded answer buttons (red, blue, green, yellow — like Kahoot)
  ...
  - Responsive design — should work on phones (friends will play on their phones)
- [ ] **Edge Cases:**
...
- [ ] **Deploy:**
  ...
  - Test the live URL end-to-end — including on your phone
...
Practice saying these naturally. They demonstrate real technical depth and design thinking — exactly what an interviewer wants to hear from a new grad.

**"Walk me through the architecture."**
> It's a two-mode trivia app. Solo mode is stateless REST — the client fetches questions, the player answers at their own pace, and the score gets posted at the end. Multiplayer is a real-time system built on Socket.io where the server acts as the authoritative game host — it controls timing, validates answers, calculates scores, and pushes state to all connected clients simultaneously. I deliberately separated the two modes because they have fundamentally different requirements, and it let me build and test solo without touching WebSockets.

**"Why is timing server-authoritative?"**
> In a competitive game where score depends on response time, you can't trust the client's clock. Two players could have different system times, or someone could manipulate their local timestamps. So the server records when it sent the question, and when each answer arrives — the response time is calculated server-side. The client countdown is just a visual aid.

**"How does the Kahoot-style scoring work?"**
> Correct answers earn between 850 and 1000 points based on response time — it's a linear interpolation over the 20-second window. I chose 850 as the floor so that a slow correct answer still significantly beats a wrong answer (which scores 0). The 150-point spread per question means speed matters across a full 15-question game, but knowledge is always more valuable than reflexes.

**"Why is the multiplayer leaderboard not persisted?"**
> Design decision. The multiplayer leaderboard only exists for that session — it's a competition between the people in the room. Persisting it would mix competitive real-time scores with the solo grind leaderboard, which are fundamentally different. Solo rewards consistency over time. Multiplayer rewards performance in a single high-pressure session. Keeping them separate makes each mode's leaderboard meaningful.

**"How do you handle a player disconnecting mid-game?"**
> Socket.io fires a `disconnect` event. The server removes that player from the room's active player list and checks if all remaining players have answered the current question — if so, it advances immediately. If the host disconnects, the next player becomes host. If only one player is left, the game continues so they can finish, but it's effectively solo at that point.
...
1. **Real-time multiplayer** — most student projects are CRUD apps. WebSockets demonstrate you understand bidirectional communication, state synchronization, and event-driven architecture.

2. **Server-authoritative game logic** — you made deliberate anti-cheat decisions (server-side timing, server-calculated scores). This shows you think about trust boundaries.

3. **AI integration** — you used an LLM API for practical content generation, not just a chatbot wrapper. You can talk about prompt engineering for structured output.

4. **Scoring algorithm design** — you can explain the math behind the 850–1000 point range and why you chose those numbers. This shows product thinking.

5. **Two distinct architectural patterns** in one app — REST for solo, WebSockets for multiplayer. You can compare them and explain when each is appropriate.

6. **Full deployment pipeline** — GitHub → Vercel/Railway with environment variables, database hosting, CI/CD. You've shipped something real, not just `localhost:3000`.
...
*Build it phase by phase, test as you go, and have fun with it. The moment two browser tabs are racing to answer the same question at the same time — that's when it clicks.*
